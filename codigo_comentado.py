...
class GolsSofridos:
    ...
    nome: str
    gols: int
def atualiza_lista_gols(time: str, gols: int, lista_estat: list[GolsSofridos]):
    """
    Procura um time na lista e atualiza seus gols. Se não encontrar, cria um novo registro.
    Esta versão não utiliza 'break'.
    """
    encontrado = False
    i = 0
    # Usamos um laço 'while' para parar a busca assim que o time é encontrado
...
def calcula_gols_sofridos_rec(jogos: list[Jogo], estat_gols: list[GolsSofridos]) -> list[GolsSofridos]:
    ...
    Segue a restrição de ter apenas um return.
    """
    resultado_final: list[GolsSofridos]

    if not jogos:
        # Caso Base: Se não houver mais jogos, o resultado é a lista atual.
        resultado_final = estat_gols
    else:
        # Passo Recursivo:
        jogo_atual = jogos[0]

        # Atualiza os gols sofridos para o anfitrião e o visitante
        ...
        # Chama a função para o resto da lista e armazena o resultado para o return final
...
def exibe_melhor_defesa(jogos_str: list[str]) -> str:
    ...
    usando apenas concatenação básica.

    >>> jogos = ["Flamengo 2 Vasco 1\\n", "Vasco 1 Botafogo 1\\n", "Botafogo 3 Flamengo 2\\n"]
    >>> exibe_melhor_defesa(jogos)
    'Time(s) com menos gols sofridos: Vasco, Botafogo (3 gols)'
    """
    string_de_saida = ""
    ...
    if not jogos_obj:
        ...
        # 2. Chama a função recursiva com uma lista vazia
        estat_final = calcula_gols_sofridos_rec(jogos_obj, [])
        
        # 3. Analisa a lista de resultados
        min_gols = -1
        # Encontra o menor número de gols sofridos
        if estat_final:
        ...
        # Encontra todos os times que correspondem ao menor número
        melhores_defesas = []
        if min_gols != -1:
        ...
        # 4. Formata a string de saída manualmente
        times_str = ""
        if melhores_defesas:
        ...
        # Constrói a string de saída usando concatenação básica
        string_de_saida = "Time com menos gols sofridos: " + times_str + " (" + str(min_gols) + " gols)"
    
    return string_de_saida
'''
Para a pergunta 3, você deve identificar o time que sofreu o menor número de gols, considerando
todos os jogos daquele time.
'''
